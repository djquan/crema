...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Simplified chroma estimate (max - min)
    let max_ch = max(r, max(g, b));
    let min_ch = min(r, min(g, b));
    let chroma = max_ch - min_ch;
    let sat = clamp(chroma / max(max_ch, 0.001), 0.0, 1.0);

    let sign = select(-1.0, 1.0, strength > 0.0);
    let effect = max(strength * (1.0 - sign * sat), -1.0);
