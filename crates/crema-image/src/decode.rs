...
pub(crate) fn jpeg(
    ...
    let icc = match decoder.icc_profile() {
        Ok(Some(_)) => Fact::Known(Icc::PresentNotApplied),
        Ok(None) => Fact::Known(Icc::Absent),
        Err(_) => Fact::Unknown(UnknownReason::MetadataInvalid),
    };
    ...
    let metadata = SourceMetadata {
        ...
        limitations: "sdr-srgb-assumption;embedded-icc-not-applied;display-profile-not-managed"
            .into(),
    };
...
pub(crate) fn heif(
    ...
    let mut limitations = "experimental-heif-srgb-conversion;icc-not-applied;hdr-not-tone-mapped;display-profile-not-managed".to_owned();
    if nclx.is_none() {
        limitations.push_str(";unknown-transfer");
    }
    let metadata = SourceMetadata {
...
mod tests {
    ...
    #[test]
    fn jpeg_reads_embedded_exif_orientation_and_icc_before_decoding() {
        let encoded = encoded_jpeg();
        ...
        let icc = b"ICC_PROFILE\0\x01\x01profile-data";
        ...
        assert_eq!(result.metadata.icc, Fact::Known(Icc::PresentNotApplied));
    }
}
